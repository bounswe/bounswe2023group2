...
def setup_test_environment():
    ...
    if TOKEN is None:
        ...
        TOKEN = login_user(TEST_USERNAME, "burak1234")
...
def test_create_resource():
    ...
    response = client.post("/resources/", json=resource_data, headers=valid_headers)
    assert response.status_code == 201
    ...
    response = client.delete(f"/resources/{resource_id}", headers=valid_headers)
    assert response.status_code == 200
...
def test_get_resource():
    ...
    resource_id = created_resource["_id"]
    response = client.get(f"/resources/{resource_id}", headers=valid_headers)
    assert response.status_code == 200
...
def test_get_all_resources():
    ...
    response = client.get("/resources/", headers=valid_headers)
    assert response.status_code == 200
...
def test_update_resource():
    ...
    response = client.put(f"/resources/{resource_id}", json=updated_resource_data, headers=valid_headers)
    assert response.status_code == 200
