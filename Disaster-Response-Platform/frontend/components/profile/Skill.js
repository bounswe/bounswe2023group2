export default function Skill({ skill, topic, deleteSelf }) {
	return (
		<div key={`${topic.key}-${skill[topic.primary]}`}>
			{
				topic.is_link ? 
				<a class="text-blue-600 text-sm" href={skill[topic.url]}>{skill[topic.text]}</a> :
			...
			<form onSubmit={deleteSelf} class="float-right">
				<input type="image" src="/icons/remove.png" class="w-5 ml-2 my-auto pt-0.5" alt="Bu girdiyi silin"/>
			</form>
		</div>
	);
}